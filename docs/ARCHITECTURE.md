...
### F0 · testbench 曾把描述符程序写死 —— **已修复**

`tb_policy_e2e.sv` 把 13 条描述符（含某一个模型的 shift）**硬编码**在
testbench 里，从不读取导出的 `instruction_program.hex`。
于是它加载导出的**权重**，却跑一个**固定的程序**——
任何 weight frac 与已部署模型不同的策略都会失败，
而已部署模型通过，只是因为它的 shift 恰好等于那些常量。

定位方法：sequence 结束后 dump vector cache（每条描述符写自己的地址，
所以每一层的输出都还在），与软件参考逐层比对 → 第一处分歧在**描述符 0**；
再对软件模型扫描假定的 shift → **shift=11 时 128/128 逐位吻合**，
而描述符要求的是 13。11 正是已部署模型该层的 shift。

**RTL 无需改动**——加速器精确执行了它被给到的 shift。
修复后两个模型都通过（均 1,799 cycle）。

> **这条 finding 的元教训**：一个只能验证一个模型的回归测试，
> 会永远通过，同时静默地忽略被测对象。
> "RTL 执行导出的程序"这件事，在此之前从未被真正检验过。
