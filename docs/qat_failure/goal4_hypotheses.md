...
### H8 —— 解除学习率钳制即可恢复 W8A8 · **已证伪**

`qat_v2_fixlr`（`--schedule fixed --learning-rate 2.563e-4`，2000/2000 步，
学习率恒定、**0.0%** 迭代在下限，优化器读回已断言）：

| arm | turn_L | turn_R | stop | height | **mean** |
|---|---|---|---|---|---|
| PTQ W8A8 | 0.758 | 0.523 | 0.984 | 0.539 | 0.401 |
| QAT W8A8 基线 | 0.000 | 0.000 | 0.938 | 1.000 | 0.501 |
| **QAT W8A8 固定 LR** | **0.000** | **0.000** | 0.023 | 0.000 | **0.206** |

**转向纹丝不动，整体还更差**（0.501 → 0.206，低于 PTQ）。
arm A（浮点执行）0.143——依然依赖量化器。

**判定：证伪。** 学习率崩塌是真实的、可复现的、可预测的，
但它**不是**控制能力丢失的原因。把步长还回去、却不加任何行为保持项，
只会让策略漂得更远——下限原来是一个**意外的刹车**。

> **一个能解释现象的机理，不是产生现象的机理。**
> 学习率崩塌与控制失败在整条精度阶梯上完全相关，
> 而在只变一个量的阶梯上，相关极易被误读为因果。**只有干预能分开它们。**

### H8b —— 激活位宽才是约束 · **已证实**

`qat_v2_w8a16`（**正常**自适应控制器，下限占比 2.8%，KL 中位 0.0055）：

| arm | 七段 success | `yaw_rmse` turn_L / turn_R |
|---|---|---|
| **fake-quant (B)** | **全部 1.000** | 0.0194 / 0.0261 |
| **浮点 (A)** | **全部 1.000** | 0.0188 / 0.0271 |

FP32 对照是 0.0224 / 0.0295——**W8A16 比它还好**。

**arm A == arm B == 1.000**：W8A16 的 QAT **没有**变成依赖量化器的策略。
它学到的是"能容忍量化"，而不是"只有在量化下才成立"。
这与 W8A8 形成最锐利的对比。
